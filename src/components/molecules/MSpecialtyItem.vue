<template>
  <div
    ...
    class="m-specialty-item"
  >
    <div class="m-specialty-item__head">
      <router-link :to="`${url}sv${faculty.code}/s${specialty.code}/`">
        <p class="m-specialty-item__title text-semibold-xl text-dark">
          {{ specialty.name }}
        </p>
      </router-link>
      <q-icon
    ...
    <p class="a-mt-8 m-specialty-item__additional-info text-regular-s">
      ({{ specialty.code_okso }} -
      {{ form.name.toLowerCase() }}, {{ specialty.level_code }},
      {{ item.is_accreditation ? 'аккредитовано' : 'не аккредитовано' }})
    </p>
    <div
      v-if="showFaculty"
      class="a-mb-24 m-specialty-item__profiles"
    >
      <p class="m-specialty-item__profiles-title text-medium-l">
        Факультет:
      </p>
      <p class="m-specialty-item__profiles-list text-regular-m">
        <router-link
          :to="`${url}faculties/${faculty.code}/`"
          class="text-dark"
        >
          {{ faculty.name }}
        </router-link>
    ...
    <div class="a-mb-24 m-specialty-item__profiles">
      <p class="m-specialty-item__profiles-title text-medium-l">
        Профили:
      </p>
      <p class="m-specialty-item__profiles-list text-regular-m">
    ...
    <div class="a-mb-16 m-specialty-item__score">
      <ASpecialtyScore
        :title="'Бюджетных'"
        :count="item.budget_places"
        :score="item.budget_score"
      />
      <ASpecialtyScore
        :count="item.commercial_places"
        :score="item.commercial_score"
      />
    </div>
    <div
      v-if="item.description"
      class="gt-xs a-mb-24 m-specialty-item__about"
    >
      <q-icon
        name="o_assistant"
        color="green"
        size="1.5rem"
        class="m-specialty-item__about-icon"
      />
      <p class="m-specialty-item__about-text text-regular-s">
        {{ item.description }}
      </p>
    </div>
    <p
      v-if="item.calculation_data?.exams"
      class="m-specialty-item__exams-list text-regular-m"
    >
      <span class="text-semibold-m">Экзамены:</span> {{ item.calculation_data.exams }}
    </p>
...
const price = computed(() => `${formatPrice(props.second ? props.item.cost_second_education : props.item.cost)}₽ в год`);
